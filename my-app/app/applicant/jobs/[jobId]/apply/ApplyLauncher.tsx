  ...
  return (
    <form onSubmit={onSubmit} className="space-y-7">
      <p className="rounded-2xl border border-blue-100/90 bg-gradient-to-br from-blue-50/90 to-white p-5 text-[13px] leading-relaxed text-slate-600 ring-1 ring-blue-500/5">
        <span className="font-semibold text-slate-900">Resume + GitHub</span> — we read your resume and your{" "}
        <strong className="font-semibold text-slate-800">repo or profile</strong> (up to five public repos, ordered by
        how well they match what you list on your resume). You will get{" "}
        <strong className="font-semibold text-slate-800">five</strong> questions that blend codebase understanding with
        engineering judgment — not trivia like reciting package scripts. Add{" "}
        <code className="rounded-md bg-white px-1.5 py-0.5 font-mono text-[11px] text-slate-800 shadow-sm">
          GITHUB_TOKEN
        </code>{" "}
        to <code className="rounded-md bg-white px-1.5 py-0.5 font-mono text-[11px] shadow-sm">.env.local</code> for
        higher API limits or private repos.
      </p>
      ...
      <div className="space-y-2">
        <label className="block text-[13px] font-medium text-slate-700">
          Resume (PDF) <span className="text-red-600">*</span> unless you use URL below
        </label>
        <label className="group flex cursor-pointer flex-col items-center justify-center gap-2 rounded-2xl border border-dashed border-slate-300 bg-slate-50/80 px-6 py-10 text-center transition-all duration-300 hover:border-blue-400/60 hover:bg-blue-50/40 hover:shadow-[0_0_0_4px_rgba(59,130,246,0.08)]">
          ...
          {fileName ? (
            ...
            <>
              ...
              <span className="text-xs text-slate-500">Or use resume URL instead</span>
            </>
          )}
          <input
            ref={fileRef}
